...
export async function apiRequest<T = unknown>(
    ...
    if (!response.ok) {
        ...
        if (response.status >= 400 && response.status < 500) {
            ...
            error.message = data?.message || data?.error || `Client Error ${response.status}`;
        } else if (response.status >= 500) {
            error.isServerError = true;
            error.message = data?.message || data?.error || `Server Error ${response.status}`;
        } else {
...
export const http = {
    ...
    postForm: <T>(path: string, formData: FormDataOptions, options?: RequestOptions) =>
    ...
    postUrlEncoded: <T>(path: string, data: Record<string, string>, options?: RequestOptions) =>
        apiRequest<T>(path, 'POST', {
            ...options,
